...
pub async fn balance(token: &str) -> monzo::Result<()> {
    ...
    let accounts_with_pots = accounts.iter().filter(|a| !a.account_number.is_empty());

    print_pot_balance_row("Account Type", "Account Number", "Pot Name", "Balance");
    println!("-------------------------------------------------------------------------------");
    for account in accounts_with_pots {
        let account_type = accounts::map_account_type(&account.account_type);
